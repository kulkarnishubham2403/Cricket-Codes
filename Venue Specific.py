...
pp_result = pandas.DataFrame(columns = ['Team', 'Wickets', 'Runs', 'Overs', 'Average', 'RPO', 'Strike-rate', 'Dot Balls', 'DBP', 'BP6', 'BP4'])
for i in df['batting_team'].unique():
    ...
    if fours>0 :
        ...
        bp4 = 0
    if finalrows/6>0 :
        df3 = pandas.DataFrame({"Team":[i], "Wickets":[wickets], "Runs":[runs], "Overs":[int(finalrows/6)+(finalrows%6)/10], "Average":[average], "RPO":[rpo], "Strike-rate":[strikerate], "Dot Balls":[dotballs], "DBP":[dbp], "BP6":[bp6], "BP4":[bp4]})
        pp_result = pp_result.append(df3)   
pp_result.sort_values(by=['RPO'], inplace=True, ascending=True)
final = pp_result.round(decimals=2)
print(final)
final.to_excel(r'C:\Cricket Adda\T20 Data\PP Bowlers\IPL PP 2021.xlsx', index = False)
